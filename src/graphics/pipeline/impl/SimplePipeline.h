...
inline void SimplePipeline::Init(const PipelineContext& ctx)
{
	...
	m_gbuffers[0] = ctx.GetRawContext()->CreateTexture(1280, 720, TEX_FORMAT_RGBA32_FLOAT, BIND_RENDER_TARGET | BIND_SHADER_RESOURCE);
	m_gbuffers[1] = ctx.GetRawContext()->CreateTexture(1280, 720, TEX_FORMAT_RGBA32_FLOAT, BIND_RENDER_TARGET | BIND_SHADER_RESOURCE);
	m_gbuffers[2] = ctx.GetRawContext()->CreateTexture(1280, 720, TEX_FORMAT_RGBA32_FLOAT, BIND_RENDER_TARGET | BIND_SHADER_RESOURCE);
	m_gbuffers[3] = ctx.GetRawContext()->CreateTexture(1280, 720, TEX_FORMAT_RGBA32_FLOAT, BIND_RENDER_TARGET | BIND_SHADER_RESOURCE);
	m_gbuffers[4] = ctx.GetRawContext()->CreateTexture(1280, 720, TEX_FORMAT_D16_UNORM, BIND_DEPTH_STENCIL | BIND_SHADER_RESOURCE);
	...
	m_shadowmap = ctx.GetRawContext()->CreateTexture(2048, 2048, TEX_FORMAT_D16_UNORM, BIND_DEPTH_STENCIL | BIND_SHADER_RESOURCE);
	...
	m_satA = ctx.GetRawContext()->CreateTexture(2048, 2048, TEX_FORMAT_RG32_FLOAT, BIND_RENDER_TARGET | BIND_SHADER_RESOURCE);
	m_satB = ctx.GetRawContext()->CreateTexture(2048, 2048, TEX_FORMAT_RG32_FLOAT, BIND_RENDER_TARGET | BIND_SHADER_RESOURCE);
	...
	m_compositeMat->GetProperties().Set("_ShadowMap", m_shadowmap->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE));
}
